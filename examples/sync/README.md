...
Server-authority effects are declared per committed Message through the journal's `effects` policy and settled through the durable ledger, keyed by the operation id, so a resend or replay cannot repeat one. Only the client exchange path settles them; an agent-path commit would need the same call.
