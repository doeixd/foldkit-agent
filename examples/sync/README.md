...
- Refusal: `refuses unauthorized writes and unauthenticated reads` and `applies the app policy against the authoritative Model`.
...
Application-owned, not covered here: authenticating the transport and deriving the `Principal` per connection (the example fixes one principal at server construction, so credential expiry has no model), Message-schema migration, effect identity across reordered policies, and retention or GC of identity rows. Browser storage eviction and abrupt power loss are outside the tests. Clock recovery — observed counters, a clock database separate from the outbox, and a fresh replica id after clock-storage loss — is in the [sync README](../../packages/sync/README.md#last-writer-wins-fields-m8-experimental).
