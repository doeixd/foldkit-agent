...
| Can existing Schemas represent the data? | `app.ts` defines Model and Message once. `sync.ts` builds `Surface.application({ Model, Message, initial, update })`, selects the shared fields with `Surface.pick(App.fields.todos)`, and marks the durable variants with `Surface.messages(...)`; `Sync.forApplication` derives the shared projection, the durable subset, the initial snapshot, and replay from that one application declaration (it compiles to `foldkit-sync`'s `defineSync`), which encodes Messages and strictly decodes operation envelopes, persisted snapshots, and exchange responses. Only `todos` is shared; selection and errors are omitted from storage. The example uses JSON-compatible fields; transformed/non-JSON schemas need explicit encoded snapshot handling before generalizing. |
