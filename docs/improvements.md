...
- **`node:sqlite` under Vite** cost time twice. Add a Vite alias for
  `node:sqlite` (or a shared `createRequire` test helper) instead of documenting
  the workaround in every new sqlite-using package.
