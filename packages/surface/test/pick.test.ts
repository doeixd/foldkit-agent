...
describe('Projection.pick', () => {
  ...
  it('installs only the declared fields', () => {
    const Pick = Projection.pick(App.model.todos)
  ...
  it('deduplicates an identical member', () => {
    ...
    expect(Projection.pick(todos, todos).dependencies).toEqual([['todos']])
    expect(Projection.pick(todos, todos).get(model)).toEqual({ todos: model.todos })
  })
  ...
  it('rejects references from a different application with an identical Model', () => {
    ...
    expect(() => Projection.pick(App.model.todos, Other.model.selectedTodoId)).toThrow(
